...
D = 0.85;
L = 1.02e-3; % Inductance in Henries (0.7875 mH)
I_max = 6.25; % Maximum current in Amperes
I_avg = 5;
I_ripple = 2.5;
I_rms = sqrt(I_avg^2+(I_ripple/2)^2/12);
J_rms = 5e6;
k = 0.5; %packing factor 
B_max = 0.228; % Maximum flux density in Tesla (228 mT)
B_sat = 0.380; % Saturation flux density in Tesla (380 mT)
Ac = Ac2; % Core cross-sectional area in m² (173 mm²)
Aw = Aw2; % Window area in m² (278.53 mm²)
l_mag = 239.1e-3; % Magnetic path length in meters 
...
function P_loss = skineffect_15kHz
% This function calculates the skin effect, AC resistance, and power loss
% for a cylindrical copper wire at 15 kHz.
...
D_wire_m    = 5.649e-04*2;      % Wire diameter [m]
f           = 15e3;           % Frequency = 15 kHz [Hz]
...
length_m    = 9.6;  % Length of wire in meters
current_A   = 5.013;  % RMS current in Amps

% Compute power loss at 15 kHz
P_loss = (current_A^2) * real(z_math) * length_m;  % Power loss in Watts

%% *Display Results*
fprintf('--- Skin Effect Analysis at 15 kHz ---\n');
